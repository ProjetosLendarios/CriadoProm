...
namespace caixaDeTexto
{
    public partial class Form1 : DevExpress.XtraEditors.XtraForm
    {
        ...
        public void ini()
        {
            mTabelaOrigem = TextBoxTabela01.Text;
            mTabelaDestino = TextBoxTabela02.Text;
            ...
            dbOrigem = GetDatabaseName(mConnectionStringOrigem);
            dbDestino = GetDatabaseName(mConnectionStringDestino);
 ...
 vDataOrigem.AddParameter(""@IGID"", System.Data.DbType.Int32, ID, dbOrigem);
    ...
    protected override string GetTableName() {{
        return ""{mTabelaDestino}"";
            ...
            TextBoxDisplay.Text = indentedCode;
        ...
        private void TextBoxTabela01_TextChanged(object sender, EventArgs e)
        {
            // Verifica se a caixa de texto não está vazia
            if (!string.IsNullOrEmpty(TextBoxTabela01.Text))
            {
                // Converte o primeiro caractere para maiúsculo
                string texto = TextBoxTabela01.Text;
                string primeiroCaractere = texto.Substring(0, 1);
                string restoDoTexto = texto.Substring(1);
                TextBoxTabela01.Text = primeiroCaractere.ToUpper() + restoDoTexto;
                // Posiciona o cursor no final do texto
                TextBoxTabela01.SelectionStart = TextBoxTabela01.Text.Length;
            }
        }
        private void Zoom(object sender, MouseEventArgs e)
        {
            if (Control.ModifierKeys == Keys.Control) // Verifica se a tecla Control está pressionada
            {
                int size = Convert.ToInt32(TextBoxDisplay.Font.SizeInPoints); // Obtém o tamanho atual da fonte
                if (e.Delta > 0) // Se a roda do mouse foi rolada para cima
                {
                    size++; // Aumenta o tamanho da fonte
                }
                else // Se a roda do mouse foi rolada para baixo
                {
                    size--; // Diminui o tamanho da fonte
                }
                TextBoxDisplay.Font = new Font(TextBoxDisplay.Font.FontFamily, size); // Define o novo tamanho da fonte
            }
        }
        private void TextBoxTabela02_TextChanged(object sender, EventArgs e)
        {
            // Verifica se a caixa de texto não está vazia
            if (!string.IsNullOrEmpty(TextBoxTabela02.Text))
            {
                // Converte o primeiro caractere para maiúsculo
                string texto = TextBoxTabela02.Text;
                string primeiroCaractere = texto.Substring(0, 1);
                string restoDoTexto = texto.Substring(1);
                TextBoxTabela02.Text = primeiroCaractere.ToUpper() + restoDoTexto;
                // Posiciona o cursor no final do texto
                TextBoxTabela02.SelectionStart = TextBoxTabela02.Text.Length;
            }
        }
        #endregion
        #region Funções
        ...
        private string MainCreateTable()
        {
